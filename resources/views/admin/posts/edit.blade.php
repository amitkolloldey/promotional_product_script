...
@section('page_title')Edit Post @endsection
@section('content')
            ...
            <div class="col-md-12 mt-3 mb-3 card">
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="general">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    <label>Content</label>
                                    <textarea name="post_content" id="page_content" cols="30" rows="10">{{old('post_content',$post['content'])}}</textarea>
                                </div>
                                <div class="form-group">
                                    <label>Featured Image</label>
                                    <input type="file" name="image">
                                    @if (isset($post['image']))
                                        <img src="{{asset('files/23/Photos/Posts/').'/'.$post['image']}}" alt="{{$post['title']}}" width="50px">
                                    @else
                                        <img src="{{asset('files/23/Photos/Posts/no_image.png')}}" alt="{{$post['title']}}" width="50px">
                                    @endif
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="status">
                                        <option value="1" @if(old('status') == "1") selected
                                                @elseif($post['status'] == '1') selected @endif>Active
                                        </option>
                                        <option value="0" @if(old('status') == "0") selected
                                                @elseif($post['status'] == '0') selected @endif>In Active
                                        </option>
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="tab-pane fade" id="meta">
                        @include('admin.includes.meta.edit', ['item' => $post])
                    </div>
                </div>
            </div>
            <div class="form-group">
